...
BattleClock.prototype = {
  ...
  // create a new battle
	...
	// end the battle due to a player dropping
	end_by_drop: function (err, dropping_player_id, cb) {
		console.log("BattleClock "+this.room_id+" drop player " + dropping_player_id + "from battle");
    	var battleClock = this;
       
      this.changeRunMode(err, 'end', function (err, isGreatSuccess) {
        battleClock.battle.dropping_player_id = dropping_player_id;
        cb(err, false);
      });
        ...
        Player.get(null, session.player_id, function (err, player) {
          ...
          if (typeof battleState !== 'undefined') {
            ...
            if (typeof battleState.player[player.id] !== 'undefined') {
              battleClock.end_by_drop(null, player.id, function () {
