...
var battleScript = [
  ...
  // post rap battle
	[function(ss, battle) {
    var numVotesLeft = battle.player[battle.players[0]].total_mad_props;
    var numVotesRight = battle.player[battle.players[1]].total_mad_props;
...
BattleClock.prototype = {
  ...
  changeState: function (err, cb) {
    ...
    if (typeof this.run_mode !== 'undefined' && this.run_mode == 'resetting') {
      ...
      // kick both players off of opentok
      
      if (typeof this.battle.left.player_id !== 'undefined') {
        Socket.players[this.battle.left.player_id].emit('stopOpenTok', 0);
      }
      if (typeof this.battle.right.player_id !== 'undefined') {
        Socket.players[this.battle.right.player_id].emit('stopOpenTok', 1);
      }
    }
...
module.exports.initGame = function(ss) {
  ...
  ss.sockets.on('connection', function(socket) {
    ...
    // emit a waiting for battle to the client if the server has recently been restarted
    if ( (serverTime.getTime() - serverStartTime.getTime()) <= 5000) {
      //socket.emit('playerAlert', 'hello ' + socket.id + ' diff: ' + (serverTime.getTime() - serverStartTime.getTime())); 
      socket.emit('stateWaitingForBattle', {});
      //socket.emit('stopOpenTok', 0); // for some reason this breaks opentok until a page refresh
    }
    ...
    socket.on('vote', function(msg) {
      console.log('socket.on:vote');
    	...
    	if ( msg == "left" ) {
    		//numVotesLeft += 1;
			  battle.player[battle.players[0]].total_mad_props++;
			  battle.round[battle.current_round].player[battle.players[0]].mad_props++;
			}
    	else if ( msg == "right" ){
    		//numVotesRight += 1;
			  battle.player[battle.players[1]].total_mad_props++;
      ...
      //console.log('player0 total: ' + battle.player[battle.players[0]].total_mad_props); 
      //console.log('player1 total: ' + battle.player[battle.players[1]].total_mad_props); 

      ss.sockets.emit('updateVotes', [battle.player[battle.players[0]].total_mad_props, battle.player[battle.players[1]].total_mad_props]);
  ...
  // end defining connection listeners
	...
	function syncState(room_id) {
		// This syncs all parts of the battle state to get everybody up to date
    battle = model.Room.battle_states[room_id];
    if (typeof battle !== 'undefined') {
		  ss.sockets.emit('updateVotes', [battle.player[battle.players[0]].total_mad_props, battle.player[battle.players[1]].total_mad_props]);
    } else {
      console.error('syncState() attempted to emit to a nonexistent battle in room_id ' + room_id);
    }
	}
