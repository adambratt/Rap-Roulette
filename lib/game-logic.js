...
var gameScript = [
	...
	[function(ss) {
    var maxBeats=8;
		ss.sockets.emit("statePreRap", { beatIndex: Math.floor(Math.random()*maxBeats+1) });
		}, 5000],
...
gameSpeed = 0.5;  // increase this value to make testing easier


function GameClock ( ss, script ) {
  ...
  this.createBattle = function(obj) {
      ...
      battle.started_at = (new Date().getTime())/1000;
			...
			model.Battle.states[battle.id] = battle;
  ...
  this.start = function () {
    ...
    // start over
      ...
      return;
		...
		var clock = this;
		setTimeout(function() { clock.changeState(); }, (duration / gameSpeed));
	}
...
module.exports.startGame = function(ss) {
	...
	var clock = new GameClock( ss, gameScript );
