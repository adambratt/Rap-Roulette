	...
	[function(ss, battle) {
        ...
        if (typeof this.battle.dropped_player_id === 'undefined') {
		      if(numVotesRight>numVotesLeft )	{
			      ...
			      this.battle.losing_player_id = this.battle.players[0];
		      } else	{
			      this.battle.winning_player_id=this.battle.players[0];
			      this.battle.losing_player_id = this.battle.players[1];
		      }
        // player was dropped
        } else {
        ...
        console.log('LOSER: '+this.battle.losing_player_id);

		  ss.sockets.emit("statePostRap", {
            winning_player_id: this.battle.winning_player_id,
            losing_player_id: this.battle.losing_player_id,
            dropped_player_id: this.battle.dropped_player_id
          });
		}, 10000]
